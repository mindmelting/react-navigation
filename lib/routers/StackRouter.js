    ...
    getStateForAction: function getStateForAction(passedAction, state) {
      ...
      if (action.type === _NavigationActions2.default.NAVIGATE && childRouters[action.routeName] !== undefined) {
        ...
        if (_childRouter) {
          ...
          var index = state.routes.length - 1;
          var parentRoute = state.routes[index];

          if (action.routeName === parentRoute.routeName && (0, _shallowEqual2.default)(action.params, parentRoute.params)) {
            _route2.key = parentRoute.key; // key needs to stay the same
            return _StateUtils2.default.replaceAtIndex(state, index, _route2);
          }
        } else {
          _route2 = {
            params: action.params,
